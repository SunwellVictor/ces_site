...
- 2025-01-27: ✅ Step 17 — Demo seeder hardening (prod-safe) completed - Enhanced `demo:seed` command with production safety measures: Added production environment guard requiring --force flag, implemented console warning displaying APP_ENV/APP_URL with confirmation prompt on production, added password masking (shows ******** unless --show-password flag used), and comprehensive testing verified local execution works normally while production requires explicit --force confirmation. Branch: fix/step-17-demo-seed-guard (commit: 70dbcd7)

# Pending
- 2025-01-27: Step 18 — Per-grant throttle (optional UX/security tweak) - Rate-limit token issuance per grant (e.g., 1/minute per grant) instead of per user. Replace per-user limiter with per-grant key (download:grant:{grant_id}), update tests to reflect per-grant window. Acceptance: issuing multiple tokens for different grants in same minute succeeds; same grant is throttled. Branch: feat/step-18-download-throttle-per-grant
